...
API_NAME=order-composite
API_URL=https://localhost:8443/order-composite
...
then
  ...
  curl -s -H "Authorization: Bearer $AT" -X POST -H "Content-Type: application/json" -d "{\"name\": \"$API_NAME\", \"identifier\": \"$API_URL\", \"scopes\": [{ \"value\": \"order:read\", \"description\": \"Read order information\"}, {\"value\": \"order:write\", \"description\": \"Update order information\"}]}" https://$TENANT/api/v2/resource-servers | jq .
fi
...
then
  ...
  curl -s -H "Authorization: Bearer $AT" -X POST  -H "Content-Type: application/json" -d "{\"client_id\":\"$READER_CLIENT_ID\",\"audience\":\"$API_URL\",\"scope\":[\"order:read\"]}" https://$TENANT/api/v2/client-grants | jq .
  echo
fi
...
then
  ...
  curl -s -H "Authorization: Bearer $AT" -X POST  -H "Content-Type: application/json" -d "{\"client_id\":\"$WRITER_CLIENT_ID\",\"audience\":\"$API_URL\",\"scope\":[\"order:read\",\"order:write\"]}" https://$TENANT/api/v2/client-grants | jq .
  echo
fi
